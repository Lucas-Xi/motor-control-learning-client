...
export function FourQuadrantCard() {
  ...
  return (
    <Card density="compact" title={t('refrigerationBench.quadrantTitle')} eyebrow="4-quadrant mode machine">
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        ...
        <div>
          ...
          <div className="mt-2 grid grid-cols-2 gap-1.5 text-caption">
            <ModeButton
              mode="cooling"
              label={t('refrigerationBench.quadrantModeCooling')}
            ...
            <ModeButton
              mode="heating"
              label={t('refrigerationBench.quadrantModeHeating')}
            ...
            <ModeButton
              mode="dehumid"
              label={t('refrigerationBench.quadrantModeDehumid')}
            ...
            <ModeButton
              mode="defrost"
              label={t('refrigerationBench.quadrantModeDefrost')}
        ...
        <div className="space-y-2">
          <div className="rounded-md border border-line-subtle bg-bg-base p-2 text-caption">
            <div className="mb-1 flex items-center gap-1.5 text-ink-muted">
              ...
              <span>{t('refrigerationBench.quadrantSwitchPath')}</span>
            </div>
          ...
          <TransitionRow label={t('refrigerationBench.quadrantValveDuration')} value={`${(trans?.durationSec ?? 0).toFixed(2)} s`} />
          <TransitionRow
            label={t('refrigerationBench.quadrantFourWayValve')}
            value={trans?.fourWayValveSwitch ? t('refrigerationBench.quadrantYesClick') : t('common.no')}
            color={trans?.fourWayValveSwitch ? 'text-accent-warn' : 'text-accent-measure'}
          />
          <TransitionRow
            label={t('refrigerationBench.quadrantEevDelta')}
            value={`${(((trans?.eevTargetDelta ?? 0) * 100)).toFixed(0)} %`}
            color={Math.abs(trans?.eevTargetDelta ?? 0) > 0.3 ? 'text-accent-warn' : 'text-accent-primary'}
          />
          <TransitionRow
            label={t('refrigerationBench.quadrantPdOvershoot')}
      ...
      <div className="mt-3 h-44">
        <SafeResponsiveContainer>
          <LineChart data={chartData} margin={{ top: 6, right: 8, bottom: 0, left: -10 }}>
            ...
            <Line yAxisId="P" type="monotone" dataKey="Pd" stroke="#ff5c7a" strokeWidth={1.8} dot={false} name={t('refrigerationBench.quadrantSeriesPd')} isAnimationActive={false} />
            <Line yAxisId="P" type="monotone" dataKey="Ps" stroke="#34d6ff" strokeWidth={1.8} dot={false} name={t('refrigerationBench.quadrantSeriesPs')} isAnimationActive={false} />
      ...
      <p className="mt-2 text-caption leading-relaxed text-ink-muted">
        {t('refrigerationBench.quadrantInsightA')}
        <span className="font-mono text-accent-fault">{formatNumber(peakPd, 2)} MPa</span>
        {t('refrigerationBench.quadrantInsightB')}
      </p>
    </Card>
  );
}

const LABEL: Record<QuadrantMode, TKey> = {
  cooling: 'refrigerationBench.quadrantModeCooling',
  heating: 'refrigerationBench.quadrantModeHeating',
  dehumid: 'refrigerationBench.quadrantModeDehumid',
  defrost: 'refrigerationBench.quadrantModeDefrost',
};
...
function QuadrantWheel({ current, previous }: { current: QuadrantMode; previous: QuadrantMode }) {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} className="w-full" role="img" aria-label={`${t('refrigerationBench.quadrantAriaPre')}${t(LABEL[current])}${t('refrigerationBench.quadrantAriaInfix')}${t(LABEL[previous])}`}>
      {/* 4 个象限扇形 */}
