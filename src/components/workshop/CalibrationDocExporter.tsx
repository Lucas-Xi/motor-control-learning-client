...
import { parameterSchemas, localizeParamSchema } from '../../content/parameterSchemas';
...
function collectRows(locale: Locale): ParamRow[] {
  ...
  // label / unit 经 localizeParamSchema 按 locale 取（缺 En 时回退中文原文）
  for (const rawSchema of Object.values(parameterSchemas)) {
    if (!rawSchema) continue;
    const schema = localizeParamSchema(rawSchema, locale);
...
function buildMarkdown(projectName: string, mcuFamily: McuFamily, locale: Locale = 'zh-CN'): string {
  // 标定单文案标签表：EN 全部半角标点；来自 store / 选型库的数据字段（brand / partNo / 策略名等）不翻译
  const L = locale === 'en-US'
    ? {
        docTitle: 'Calibration Sheet',
        titleSep: ' - ',
        colItem: 'Item', colValue: 'Value',
        rowDate: 'Date', rowMcu: 'MCU part',
        rowCompressor: 'Compressor', rowInverter: 'Inverter platform',
        rowStrategy: 'Control strategy', rowLoad: 'Load condition',
        rowPfc: 'PFC front end', rowSeparator: 'Liquid separator',
        noRunSlots: '(no run yet - run the full-machine simulation in the workshop first)',
        paramSectionTitle: 'Key Parameter Calibration Table',
        colParam: 'Parameter', colCurrent: 'Current', colUnit: 'Unit',
        colRange: 'Recommended range', colInRange: 'In range',
        inRangeYes: 'in range', inRangeNo: 'OUT OF RANGE',
        scoreSectionTitle: 'Current Combination Score',
        rowVerdict: 'Overall verdict',
        verdictPass: 'PASS', verdictPassWarn: 'PASS with warnings', verdictFail: 'FAIL',
        rowTd: 'Discharge temp Td', rowReached: 'Target met',
        yes: 'yes', no: 'no',
        rowFaultCount: 'Faults', rowWarnCount: 'Warnings',
        noRunRecord: '(No run record yet. Run the full-machine simulation in the workshop before generating this sheet.)',
        challengeSectionTitle: 'Challenge Completion Records',
        noChallenge: '(no completed challenges yet)',
        colChallengeId: 'Challenge ID', colAttempts: 'Attempts',
        colBest: 'Best value', colFirstPassed: 'First passed at',
        footerGenerated: (d: string) => `Auto-generated by the Motor Control Learning Client - ${d}`,
        footerNote1: '> This file is the calibration baseline. To reproduce it, restore the six selections (compressor + inverter + control',
        footerNote2: '> strategy + load + PFC + separator) in the workshop, then check every store slider value against the key parameter calibration table. **All recommended ranges come from `parameterSchemas.ts`**.',
        sliceTitles: {
          motorBasics: 'Motor model (motorBasics)',
          foc: 'FOC current loop (foc)',
          pid: 'PID speed loop (pid)',
          controlLoop: 'Cascaded three-loop (controlLoop)',
          svpwm: 'SVPWM modulation (svpwm)',
          inverter: 'Inverter (inverter)',
          startup: 'Startup state machine (startup)',
          apf: 'PFC front end (apf)',
          refrigeration: 'Refrigeration cycle (refrigeration)',
        } as Record<SliceKey, string>,
      }
    : {
        docTitle: '标定单',
        titleSep: ' · ',
        colItem: '项目', colValue: '值',
        rowDate: '日期', rowMcu: 'MCU 型号',
        rowCompressor: '压缩机', rowInverter: '变频器平台',
        rowStrategy: '控制策略', rowLoad: '工况负载',
        rowPfc: 'PFC 前级', rowSeparator: '液气分离器',
        noRunSlots: '（未运行过 → 请到工作台跑一次"运行整机仿真"）',
        paramSectionTitle: '关键参数标定表',
        colParam: '参数', colCurrent: '当前值', colUnit: '单位',
        colRange: '推荐范围', colInRange: '是否在区间',
        inRangeYes: '✓ 在区间', inRangeNo: '✗ 超界',
        scoreSectionTitle: '当前组合评分',
        rowVerdict: '总判定',
        verdictPass: '✓ 通过', verdictPassWarn: '⚠ 通过 · 有告警', verdictFail: '✗ 不通过',
        rowTd: '排气温度 Td', rowReached: '达标',
        yes: '✓ 是', no: '✗ 否',
        rowFaultCount: '故障数', rowWarnCount: '告警数',
        noRunRecord: '（暂无运行记录。请到工作台执行"运行整机仿真"后再生成。）',
        challengeSectionTitle: '挑战通关记录',
        noChallenge: '（暂无通关记录）',
        colChallengeId: '挑战 ID', colAttempts: '尝试次数',
        colBest: '最优指标', colFirstPassed: '首次通关时间',
        footerGenerated: (d: string) => `自动生成 · 电机控制学习客户端 · ${d}`,
        footerNote1: '> 此文件作为标定基线。如要复现，在工作台还原"压缩机 + 变频器 + 控制策略 + 工况 + PFC + 分离器"6 项选型，',
        footerNote2: '> 然后按"关键参数标定表"逐项校对 store 滑块值即可。**所有推荐范围来自 `parameterSchemas.ts`**。',
        sliceTitles: {
  ...
  const head = [
    `# ${projectName}${L.titleSep}${L.docTitle}`,
    '',
    `| ${L.colItem} | ${L.colValue} |`,
    '| --- | --- |',
    `| ${L.rowDate} | ${dateStr} |`,
    `| ${L.rowMcu} | ${mcuFamily} |`,
    `| ${L.rowCompressor} | ${compressorLabel} |`,
    `| ${L.rowInverter} | ${inverterLabel} |`,
    `| ${L.rowStrategy} | ${strategyLabel} |`,
    `| ${L.rowLoad} | ${loadLabel} |`,
    `| ${L.rowPfc} | ${pfcLabel} |`,
    `| ${L.rowSeparator} | ${sepLabel} |`,
    '',
  ];

  // 参数表 - 按 slice 分组
  const paramSections: string[] = [`## ${L.paramSectionTitle}`, ''];
  for (const slice of TARGET_SLICES) {
    const sliceRows = rows.filter((r) => r.slice === slice);
    if (sliceRows.length === 0) continue;
    paramSections.push(`### ${L.sliceTitles[slice]}`, '');
    paramSections.push(`| ${L.colParam} | ${L.colCurrent} | ${L.colUnit} | ${L.colRange} | ${L.colInRange} |`);
    paramSections.push('| --- | ---: | --- | --- | :---: |');
    for (const r of sliceRows) {
      const valueStr = typeof r.value === 'number'
        ...
        : String(r.value);
      paramSections.push(
        `| ${r.label} (\`${r.key}\`) | ${valueStr} | ${r.unit || '—'} | ${r.min} ~ ${r.max} | ${r.inRange ? L.inRangeYes : L.inRangeNo} |`,
      );
    }
    paramSections.push('');
  }
  ...
  // 评分 + 当前组合 verdict
  const verdictSection: string[] = [`## ${L.scoreSectionTitle}`, ''];
  if (last) {
    const verdictText = last.verdict === 'pass' ? L.verdictPass
      : last.verdict === 'pass-warn' ? L.verdictPassWarn
      : L.verdictFail;
    verdictSection.push(`| KPI | ${L.colValue} |`, '| --- | ---: |');
    verdictSection.push(`| ${L.rowVerdict} | **${verdictText}** |`);
    verdictSection.push(`| COP | ${last.cop.toFixed(2)} |`);
    verdictSection.push(`| ${L.rowTd} | ${last.Tdischarge.toFixed(1)} °C |`);
    verdictSection.push(`| ${L.rowReached} | ${last.reachedTarget ? L.yes : L.no} |`);
    verdictSection.push(`| ${L.rowFaultCount} | ${last.faultCount} |`);
    verdictSection.push(`| ${L.rowWarnCount} | ${last.warnCount} |`);
    verdictSection.push('');
  } else {
    verdictSection.push(L.noRunRecord, '');
  }

  // 挑战通关
  const challengeSection: string[] = [`## ${L.challengeSectionTitle}`, ''];
  const solvedEntries = Object.entries(challenges).filter(([, r]) => r.solved);
  if (solvedEntries.length === 0) {
    challengeSection.push(L.noChallenge, '');
  } else {
    challengeSection.push(`| ${L.colChallengeId} | ${L.colAttempts} | ${L.colBest} | ${L.colFirstPassed} |`, '| --- | ---: | ---: | --- |');
...
export function CalibrationDocExporter() {
  ...
  const handleDownloadMd = () => {
    const md = buildMarkdown(projectName, mcuFamily, locale);
    const safeName = projectName.replace(/[\\/:*?"<>|\s]+/g, '_').slice(0, 32) || 'calibration';
    const sheetWord = locale === 'en-US' ? 'calibration-sheet' : '标定单';
    downloadText(`${safeName}_${sheetWord}_${timestamp()}.md`, md, 'text/markdown;charset=utf-8');
  };
  ...
  const handleDownloadZip = () => {
    ...
    downloadBinary(`${safeName}_${sheetWord}_${timestamp()}.zip`, bin, 'application/zip');
  };
