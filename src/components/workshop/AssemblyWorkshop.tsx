...
type TopologySlotKey = 'load' | 'separator' | 'compressor' | 'pfc' | 'inverter' | 'strategy';

function TopologyDiagram({
  compressor, inverter, strategy, load, pfc, separator, refrigerantMismatch,
  ...
  // 当前打开的 popover slot；null = 关闭
  const [openSlot, setOpenSlot] = useState<TopologySlotKey | null>(null);
  ...
  return (
    <div className="rounded-xl border border-line-subtle bg-bg-base p-4">
      <div className="mb-2 flex items-center justify-between">
        <p className="text-caption uppercase tracking-[0.18em] text-ink-muted">System Topology · 点积木块直接切 slot</p>
        {refrigerantMismatch && (
      ...
      {/* 两行拓扑：上 = 制冷链路（机械），下 = 电气链路 */}
      <div className="relative space-y-1">
        <div className="grid grid-cols-5 items-center gap-1 text-caption">
          <BlockBox label="工况" sub={load.name.replace(/^[^·]+·/, '').trim()} tone="warn" active={openSlot === 'load'} onClick={() => setOpenSlot(openSlot === 'load' ? null : 'load')} acceptCategory="load" onSlotDrop={onSwapLoad} />
          <Arrow label="τ_load" sub="N·m" />
          <BlockBox label="分离器" sub={separator.id === 'none' ? '无' : separator.maxRampRpmS + ' rpm/s'} tone={separator.id === 'none' ? 'warn' : 'measure'} active={openSlot === 'separator'} onClick={() => setOpenSlot(openSlot === 'separator' ? null : 'separator')} acceptCategory="separator" onSlotDrop={onSwapSeparator} />
          <Arrow label="液滴↓" sub="" />
          <BlockBox label="压缩机" sub={compressor.partNo} tone="measure" active={openSlot === 'compressor'} onClick={() => setOpenSlot(openSlot === 'compressor' ? null : 'compressor')} acceptCategory="compressor" onSlotDrop={onSwapCompressor} />
        </div>
        {/* 中间连接：变频器输出三相电流流入压缩机 */}
        <div className="flex justify-end pr-[10%] text-[10px] text-ink-muted">
          <span className="rounded border border-accent-primary/40 px-1 py-0.5 text-accent-primary">↑ Iabc 三相</span>
        </div>
        <div className="grid grid-cols-5 items-center gap-1 text-caption">
          <BlockBox label="PFC" sub={pfc.id === 'none' ? '无' : pfc.vdcOutput + 'V'} tone={pfc.meetsHarmonicStandard ? 'primary' : 'warn'} active={openSlot === 'pfc'} onClick={() => setOpenSlot(openSlot === 'pfc' ? null : 'pfc')} acceptCategory="pfc" onSlotDrop={onSwapPfc} />
          <Arrow label="Vdc" sub="V" />
          <BlockBox label="变频器" sub={inverter.ipmPartNo} tone="primary" active={openSlot === 'inverter'} onClick={() => setOpenSlot(openSlot === 'inverter' ? null : 'inverter')} acceptCategory="inverter" onSlotDrop={onSwapInverter} />
          <Arrow label="duty" sub="0..1" reverse />
          <BlockBox label="控制器" sub={strategy.id.includes('hfi') ? 'HFI+BEMF' : strategy.id.includes('bemf') ? 'BEMF' : strategy.id.includes('encoder') ? '编码器' : 'V/f 开环'} tone="primary" active={openSlot === 'strategy'} onClick={() => setOpenSlot(openSlot === 'strategy' ? null : 'strategy')} acceptCategory="strategy" onSlotDrop={onSwapStrategy} />
        </div>

        {/* 弹出选项：选完关闭 */}
        {openSlot === 'load' && <SlotPopover title="切换工况" options={loadConditions.map((l) => ({ key: l.id, label: l.name, sub: l.brief }))} currentKey={load.id} onPick={(_, i) => { onSwapLoad(i); setOpenSlot(null); }} onClose={() => setOpenSlot(null)} />}
        {openSlot === 'separator' && <SlotPopover title="切换液气分离器" options={liquidSeparators.map((s) => ({ key: s.id, label: s.name, sub: s.brief }))} currentKey={separator.id} onPick={(_, i) => { onSwapSeparator(i); setOpenSlot(null); }} onClose={() => setOpenSlot(null)} />}
        {openSlot === 'compressor' && <SlotPopover title="切换压缩机" options={compressorBundles.map((b) => ({ key: b.id, label: `${b.compressor.brand} ${b.compressor.partNo}`, sub: `${b.compressor.type} · ${b.compressor.hp}HP · ${b.compressor.refrigerant}` }))} currentKey={compressorBundles.find((b) => b.compressor.partNo === compressor.partNo)?.id ?? ''} onPick={(_, i) => { onSwapCompressor(i); setOpenSlot(null); }} onClose={() => setOpenSlot(null)} />}
        {openSlot === 'pfc' && <SlotPopover title="切换 PFC 前级" options={pfcPlatforms.map((p) => ({ key: p.id, label: p.name, sub: p.brief }))} currentKey={pfc.id} onPick={(_, i) => { onSwapPfc(i); setOpenSlot(null); }} onClose={() => setOpenSlot(null)} />}
        {openSlot === 'inverter' && <SlotPopover title="切换变频器平台" options={inverterPlatforms.map((p) => ({ key: p.ipmPartNo, label: `${p.ipmBrand} ${p.ipmPartNo}`, sub: `${p.topology} · ${p.ratedCurrentA}A / ${p.ratedBusV}V · MCU ${p.mcuPartNo}` }))} currentKey={inverter.ipmPartNo} onPick={(_, i) => { onSwapInverter(i); setOpenSlot(null); }} onClose={() => setOpenSlot(null)} />}
        {openSlot === 'strategy' && <SlotPopover title="切换控制策略" options={controlStrategies.map((s) => ({ key: s.id, label: s.name, sub: s.brief }))} currentKey={strategy.id} onPick={(_, i) => { onSwapStrategy(i); setOpenSlot(null); }} onClose={() => setOpenSlot(null)} />}
      </div>

      <div className="mt-2 grid grid-cols-2 gap-2 text-caption text-ink-muted sm:grid-cols-4">
        <SpecBadge label="冷媒" value={`${compressor.refrigerant} ↔ ${load.refrigerant}`} highlight={refrigerantMismatch ? 'fault' : 'measure'} />
        <SpecBadge label="目标转速" value={`${load.targetRpm} rpm`} />
        <SpecBadge label="Vdc / 调制" value={`${pfc.vdcOutput}V · ${strategy.modulationLimitFactor === 0.5 ? 'SPWM' : 'SVPWM'}`} />
        <SpecBadge label="斜坡限 / 工况" value={`${separator.maxRampRpmS} vs ${load.rampRpmS} rpm/s`} highlight={load.rampRpmS > separator.maxRampRpmS ? 'fault' : 'measure'} />
      </div>
    </div>
  );
}

function BlockBox({ label, sub, tone, active, onClick, acceptCategory, onSlotDrop }: {
  label: string;
  sub: string;
  tone: 'primary' | 'measure' | 'warn';
  active?: boolean;
  onClick?: () => void;
  /** 当此块作为 drop target 时声明可接受的 category；与 SlotPicker.dragCategory 对应 */
  acceptCategory?: 'compressor' | 'inverter' | 'strategy' | 'load' | 'pfc' | 'separator';
  /** drop 时由父组件传入：拿到 chip 的 index 即可切换该 slot */
  onSlotDrop?: (idx: number) => void;
}) {
  const [dragOver, setDragOver] = useState(false);
  const toneClass = active || dragOver
    ? 'border-accent-primary ring-2 ring-accent-primary/40 bg-accent-primary/15 text-accent-primary'
    : tone === 'measure' ? 'border-accent-measure/60 bg-accent-measure/10 text-accent-measure'
    : tone === 'warn' ? 'border-accent-warn/60 bg-accent-warn/10 text-accent-warn'
    : 'border-accent-primary/60 bg-accent-primary/10 text-accent-primary';
  const className = `rounded-md border px-1.5 py-1.5 text-center transition-all ${toneClass} ${onClick ? 'cursor-pointer hover:scale-[1.02] hover:shadow-md' : ''}`;
  const children = (
    <>
      <div className="font-medium">{label}</div>
      <div className="mt-0.5 truncate text-[10px] opacity-80">{sub}</div>
      {dragOver && <div className="mt-0.5 text-[9px] uppercase tracking-wider text-accent-primary">drop ↓</div>}
    </>
  );

  // drop 处理：当且仅当 dataTransfer 里有 DRAG_TYPE 且 category 匹配时接受
  const dropHandlers = acceptCategory && onSlotDrop ? {
    ...
    onDragOver: (e: React.DragEvent) => {
      ...
      // dataTransfer.getData 在 dragOver 阶段读不到内容（只 dragstart/drop 可），
      // 但能从 types 看到自定义 MIME 存在；具体 category 校验放到 onDrop
    ...
    onDrop: (e: React.DragEvent) => {
      ...
      if (!Number.isNaN(idx)) onSlotDrop(idx);
    },
  } : {};

  if (onClick) {
    return (
      <button type="button" className={className} onClick={onClick} title={`点击切换 或 拖下方 ${label} 库的 chip 至此处`} {...dropHandlers}>
        {children}
      </button>
    );
  }
  return <div className={className} title={sub} {...dropHandlers}>{children}</div>;
}

// 顶部漂浮的选项 popover —— 用 SVG-grid 位置 + max-height 滚动
function SlotPopover({ title, options, currentKey, onPick, onClose }: {
  title: string;
  ...
  return (
    <>
      ...
      <div role="dialog" aria-label={title} className="absolute left-1/2 top-1/2 z-20 max-h-[70%] w-[90%] max-w-md -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded-xl border border-accent-primary/60 bg-bg-raised p-3 shadow-xl">
        <div className="mb-2 flex items-center justify-between">
          <p className="text-caption uppercase tracking-[0.18em] text-ink-muted">{title}</p>
...
function Arrow({ label, sub, reverse }: { label: string; sub: string; reverse?: boolean }) {
  return (
    <div className="text-center text-ink-muted">
      <div className="text-[10px] uppercase">{label}</div>
      <div className="font-mono text-body text-ink-secondary">{reverse ? '←' : '→'}</div>
      <div className="text-[10px]">{sub}</div>
    </div>
  );
}

function SpecBadge({ label, value, highlight }: { label: string; value: string; highlight?: 'measure' | 'fault' }) {
  const cls = highlight === 'fault' ? 'text-accent-fault' : highlight === 'measure' ? 'text-accent-measure' : 'text-ink-secondary';
  return (
    <div className="rounded-md border border-line-subtle bg-bg-surface px-2 py-1">
      <div className="text-[10px] uppercase text-ink-muted">{label}</div>
      <div className={`font-mono text-caption ${cls}`}>{value}</div>
    </div>
  );
}
